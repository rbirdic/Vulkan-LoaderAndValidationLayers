...
TEST_F(VkLayerTest, CommandQueueFlags) {
    ...
    VkDeviceObj device(0, gpu());

    uint32_t queueFamilyIndex = device.QueueFamilyWithoutCapabilities(VK_QUEUE_GRAPHICS_BIT);
    if(queueFamilyIndex == UINT32_MAX) {
        printf("             Non-graphics queue family not found; skipped.\n");
        return;
    } else {
        // Create command pool on a non-graphics queue
        VkCommandPoolObj command_pool(&device, queueFamilyIndex);

        // Setup command buffer on pool
        VkCommandBufferObj command_buffer(&device, &command_pool);
